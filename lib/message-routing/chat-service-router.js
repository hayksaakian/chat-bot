...
class ChatServiceRouter {
  ...
  create() {
    ...
    this.punishmentStream.on('data', (punishmentObject) => {
      // added an early return because for our simple bot we don't care about this.
      return
      ...
      if (!_.isEmpty(punishmentObject.reason) && this.chatToConnectTo === 'dgg') {
        // If it's a MEGAnuke dont output the reason messages to chat
        if (punishmentObject.type === 'ban' && punishmentObject.isNuke === true) {
          return;
        }
